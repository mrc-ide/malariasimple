...
kD <- user()
dim(age_vector) <- na
age_vector[] <- user() # vector of age categories supplied by user
...
rain_input <- daily_rain_input[day]
...
alpha_smc_array[,,] <- smc_mask[i,j,k] * alpha_smc[iteration] #Multiply by proportion of individuals in SMC compartment currently receiving SMC
...
FOI_smc[,,] <- FOI[i,j,k] * (1 - (smc_mask[i,j,k] * P_smc[day]))
...
d_itn <- dn0*itn_decay_daily[day]*itn_eff_cov_daily[day]
r_itn <- (rnm + (rn - rnm)*itn_decay_daily[day])*itn_eff_cov_daily[day] #Bednet repellency does not decay to zero.
s_itn <- 1 - d_itn - r_itn
...
min_age_prev[] <- user(integer=TRUE)
max_age_prev[] <- user(integer=TRUE)
...
min_age_inc[] <- user(integer=TRUE)
max_age_inc[] <- user(integer=TRUE)
