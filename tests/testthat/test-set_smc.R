...
test_that("get_smc_usage_mat returns zero usage from zero coverage input",{
  smc_usage_mat_random <- get_smc_usage_mat(smc_days = c(10,100,200),smc_cov = c(0,0,0),n_days = 300,distribution_type = "random")
  smc_usage_mat_cohort <- get_smc_usage_mat(smc_days = c(10,100,200),smc_cov = c(0,0,0),n_days = 300,distribution_type = "cohort")
...
test_that("alpha_smc is zero when drug_efficacy is zero",{
  params <- get_parameters(n_days = 200) |>
    set_smc(smc_days = c(1,50,99), smc_cov = 0.5, drug_efficacy = 0)
...
test_that("rel_c_days works at edge cases", {
  ...
  params_1 <- get_parameters(n_days = 200) |>
    set_smc(smc_days = smc_days, smc_cov = 0.5, drug_rel_c = 1)
  expect_true(all(params_1$rel_c_days == 1))

  params_0 <- get_parameters(n_days = 200) |>
    set_smc(smc_days = smc_days, smc_cov = 0.5, drug_rel_c = 0)
  expect_true(all(params_0$rel_c_days[smc_days+1] == 0))
