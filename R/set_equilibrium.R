...
set_equilibrium <- function(params, init_EIR)
{
  ...
  } else if (params$itn_set == 1 & params$smc_set == 1) {
    num_int <- 4
  ...
  ## Check Parameters
  if (!is.numeric(init_EIR) |
      init_EIR < 0)
    stop("init_EIR must be a numeric value greater than zero")
  ...
  rel_foi <- exp(-params$sigma2 / 2 + sqrt(params$sigma2) * het_x) / sum(het_wt * exp(-params$sigma2 /
                                                                                        2 + sqrt(params$sigma2) * het_x))
  ...
  for (i in 2:na)
  {
    age20i[i] <- ifelse(age_vector[i] >= (20 * params$DY) &
                          age_vector[i - 1] < (20 * params$DY),
                        i,
                        age20i[i - 1])
  }
  ...
  for (j in 1:nh)
  {
    for (i in 1:na)
    {
      init_IB[i, j] <- (ifelse(i == 1, 0, init_IB[i - 1, j]) +
                          EIR_eq[i, j] / (EIR_eq[i, j] * params$uB + 1) * x_I[i]) /
        (1 + x_I[i] / params$dB)
      FOI_eq[i, j] <- EIR_eq[i, j] * ifelse(init_IB[i, j] == 0, params$b0, params$b0 * ((1 - params$b1) /
                                                                                          (
                                                                                            1 + (init_IB[i, j] / params$IB0)^params$kB
                                                                                          ) + params$b1))
      init_ID[i, j] <- (ifelse(i == 1, 0, init_ID[i - 1, j]) +
                          FOI_eq[i, j] / (FOI_eq[i, j] * params$uD + 1) * x_I[i]) /
        (1 + x_I[i] / params$dID)
      init_ICA[i, j] <- (ifelse(i == 1, 0, init_ICA[i - 1, j]) +
                           FOI_eq[i, j] / (FOI_eq[i, j] * params$uCA + 1) * x_I[i]) /
        (1 + x_I[i] / params$dCA)
      p_det_eq[i, j] <- params$d1 + (1 - params$d1) / (1 + fd[i] * (init_ID[i, j] /
                                                                      params$ID0)^params$kD)
  ...
  for (j in 1:nh)
  {
    for (i in 1:na)
    {
      ...
      init_ICM[i, j] <- ifelse(i == 1, ICM_init_eq[j], init_ICM[i - 1, j]) /
        (1 + x_I[i] / params$dCM)
    }
  }
  ...
  betaT <- matrix(rep(params$rT + gamma, rep(nh, na)),
                  ncol = nh,
                  byrow = TRUE)
  betaD <- matrix(rep(params$rD + gamma, rep(nh, na)),
                  ncol = nh,
                  byrow = TRUE)
  betaP <- matrix(rep(params$rP + gamma, rep(nh, na)),
                  ncol = nh,
                  byrow = TRUE)
    ...
    for (i in 2:na)
    {
      Z_eq[i, j, 1] <- (den_het[i, j] - delta[i] * (
        Z_eq[i - 1, j, 2] / betaT[i, j] +
          Z_eq[i - 1, j, 3] /
          betaD[i, j] +
          (params$rT *  Z_eq[i - 1, j, 2] /
             betaT[i, j]
           + Z_eq[i - 1, j, 4]) /
          betaP[i, j]
      )) / (1 + aT[i, j] + aD[i, j] + aP[i, j])
      ...
      Z_eq[i, j, 4] <- aP[i, j] * Z_eq[i, j, 1] + delta[i] * (params$rT *
                                                                Z_eq[i - 1, j, 2] /
                                                                betaT[i, j] + Z_eq[i - 1, j, 4]) / betaP[i, j]
  ...
  for (i in 1:na)
  {
    ...
    {
      init_A[i, j] <- (
        delta[i] * ifelse(i == 1, 0, init_A[i - 1, j]) +
          FOI_eq[i, j] * (1 - phi_eq[i, j]) * init_Y[i, j] +
          params$rD * init_D[i, j]
      ) / (betaA[i, j] + FOI_eq[i, j] * (1 - phi_eq[i, j]))
      init_U[i, j] <- (params$rA * init_A[i, j] + delta[i] * ifelse(i == 1, 0, init_U[i - 1, j])) /
        betaU[i, j]
      init_S[i, j] <- init_Y[i, j] - init_A[i, j] - init_U[i, j]
      FOIvij_eq[i, j] <- foi_age[i] * params$av0 * (params$cT * init_T[i, j] + params$cD *
                                                      init_D[i, j] + cA_eq[i, j] * init_A[i, j] + params$cU * init_U[i, j]) * rel_foi[j] /
        omega
    }
  }
  ...
  # larval states
  K0 <- 2 * mv0 * params$dLL * params$mum * (1 + params$dPL * params$muPL) * params$gammaL * (params$lambda + 1) /
    (params$lambda / (params$muLL *
                        params$dEL) - 1 /
       (params$muLL * params$dLL) - 1)
  init_PL <- 2 * params$dPL * params$mum * mv0
  init_LL <- params$dLL * (params$muPL + 1 / params$dPL) * init_PL
  init_EL <- (init_LL / params$dLL + params$muLL * init_LL * (1 + params$gammaL * init_LL /
                                                                K0)) / (1 / params$dEL - params$muLL * params$gammaL * init_LL / K0)
  ...
  zetas <- rlnorm(
    n = 1e5,
    meanlog = -params$sigma2 / 2,
    sdlog = sqrt(params$sigma2)
  )
  while (sum(zetas > 100) > 0) {
    zetas[zetas > 100] <- rlnorm(
      n = sum(zetas > 100),
      meanlog = -params$sigma2 / 2,
      sdlog = sqrt(params$sigma2)
    )
  ...
  res <- list(
    ...
    betaU = betaU,
    FOIvij_eq = FOIvij_eq,
    age_mid_point = age_mid_point,
    het_bounds = het_bounds,
    pi = pi,
    age20l = age20l,
    age20u = age20u,
    age_20_factor = age_20_factor
  )
