...
Simulations can also be run with stochastic human transitions. To achieve this, we can just add `stochastic = TRUE` to the `get_parameters()` function call. We can also run multiple simulations by setting the `n_particles` parameter in `run_simulation()`. This will produce a 3D array with dimensions [*timesteps, variables, particles*]. 
...
##------------------- Run simulation and plot ---------------------
stochastic_output <- run_simulation(stoch_params, n_particles = n_particles)
stochastic_output_2d <- do.call(rbind, lapply(1:n_particles, function(i) stochastic_output[,,i])) |>
  as.data.frame()
stochastic_output_2d$particle <- rep(1:n_particles, each = n_days)

library(ggplot2)
ggplot() +
  geom_line(data = stochastic_output_2d, aes(x=time,y=n_detect_730_3650/n_730_3650, col = "Stochastic", group = particle)) +
