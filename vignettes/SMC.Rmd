...
vignette: >
  %\VignetteIndexEntry{Seasonal_Malaria_Chemoprevention}
...
To fully define an SMC intervention, we can specify:
 ...
 - SMC Drug (or manually define drug parameters): `drug`
 - Days on which SMC administration events occur: `smc_days`
 - Proportion of age-eligible children covered: `smc_cov`
 - Minimum age eligible for SMC: `smc_lower_age`
 - Maximum age eligible for SMC: `smc_lower_age`
...
basic_params <- get_parameters(n_days = n_days,
                               age_vector = c(0,1,2,3,4,5,10,20,40,60,80)*365) |>
  set_seasonality(season_params) |>
  set_equilibrium(init_EIR = 10)
basic_out <- run_simulation(basic_params) |> as.data.frame()

## ---------- Seasonal transmission with 'random' SMC distribution --------
smc_random_params <- basic_params |>
  set_smc(smc_cov = smc_cov, smc_days = smc_days,
          smc_lower_age = 1*365,
          smc_upper_age = 5*365,
          distribution_type = "random",
          drug = "AL") |>
  set_equilibrium(init_EIR = 10)
smc_random_out <- run_simulation(smc_random_params) |> as.data.frame()
...
smc_cohort_params <- basic_params |>
  set_smc(smc_cov = smc_cov, smc_days = smc_days,
          ...
          drug = "AL") |>
  set_equilibrium(init_EIR = 10)
smc_cohort_out <- run_simulation(smc_cohort_params) |> as.data.frame()
