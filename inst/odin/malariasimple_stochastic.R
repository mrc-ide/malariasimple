...
S_trans[, , , 2:5] <- Binomial(S_leave[i, j, k] - sum(S_trans[i, j, k, 1:(l - 1)]), S_rates[i, j, k, l] /
                                 sum(S_rates[i, j, k, l:5]))
...
T_trans[, , , 2:3] <- Binomial(T_leave[i, j, k] - sum(T_trans[i, j, k, 1:(l - 1)]), T_rates[i, j, k, l] /
                                 sum(T_rates[i, j, k, l:3]))
...
D_trans[, , , 2:3] <- Binomial(D_leave[i, j, k] - sum(D_trans[i, j, k, 1:(l - 1)]), D_rates[i, j, k, l] /
                                 sum(D_rates[i, j, k, l:3]))
...
A_trans[, , , 2:5] <- Binomial(A_leave[i, j, k] - sum(A_trans[i, j, k, 1:(l - 1)]), A_rates[i, j, k, l] /
                                 sum(A_rates[i, j, k, l:5]))
...
U_trans[, , , 2:6] <- Binomial(U_leave[i, j, k] - sum(U_trans[i, j, k, 1:(l - 1)]), U_rates[i, j, k, l] /
                                 sum(U_rates[i, j, k, l:6]))
...
P_trans[, , , 2:3] <- Binomial(P_leave[i, j, k] - sum(P_trans[i, j, k, 1:(l - 1)]), P_rates[i, j, k, l] /
                                 sum(P_rates[i, j, k, l:3]))
...
#------------------------------ TRANSITIONS ----------------------------------
update(S[1, 1:nh, 1:num_int]) <- S[i, j, k] + PS_trans[i, j, k] + US_trans[i, j, k] + U_smc_cleared[i, j, k] +
  A_smc_cleared[i, j, k] + D_smc_cleared[i, j, k] - ST_trans[i, j, k] - SD_trans[i, j, k] - SA_trans[i, j, k] -
  S_death[i, j, k] - S_age[i, j, k] + births[1, j, k]
update(S[2:na, 1:nh, 1:num_int]) <- S[i, j, k] + PS_trans[i, j, k] + US_trans[i, j, k] + U_smc_cleared[i, j, k] +
  A_smc_cleared[i, j, k] + D_smc_cleared[i, j, k] - ST_trans[i, j, k] - SD_trans[i, j, k] - SA_trans[i, j, k] -
  S_death[i, j, k] - S_age[i, j, k] + S_age[i - 1, j, k]

update(T[1, 1:nh, 1:num_int]) <- T[i, j, k] + AT_trans[i, j, k] + ST_trans[i, j, k] + UT_trans[i, j, k] -
  TP_trans[i, j, k] - T_age[i, j, k] - T_death[i, j, k]
update(T[2:na, 1:nh, 1:num_int]) <- T[i, j, k] + AT_trans[i, j, k] + ST_trans[i, j, k] + UT_trans[i, j, k] -
  TP_trans[i, j, k] - T_age[i, j, k] - T_death[i, j, k] + T_age[i - 1, j, k]

update(D[1, 1:nh, 1:num_int]) <- D[i, j, k] + SD_trans[i, j, k] + AD_trans[i, j, k] + UD_trans[i, j, k] -
  DA_trans[i, j, k] - D_smc_cleared[i, j, k] - D_death[i, j, k] - D_age[i, j, k]
update(D[2:na, 1:nh, 1:num_int]) <- D[i, j, k] + SD_trans[i, j, k] + AD_trans[i, j, k] + UD_trans[i, j, k] -
  DA_trans[i, j, k] - D_smc_cleared[i, j, k] - D_death[i, j, k] - D_age[i, j, k] + D_age[i - 1, j, k]

update(A[1, 1:nh, 1:num_int]) <- A[i, j, k] + SA_trans[i, j, k] + DA_trans[i, j, k] + UA_trans[i, j, k] -
  AT_trans[i, j, k] - AD_trans[i, j, k] - AU_trans[i, j, k] - A_smc_cleared[i, j, k] - A_death[i, j, k] - A_age[i, j, k]
update(A[2:na, 1:nh, 1:num_int]) <- A[i, j, k] + SA_trans[i, j, k] + DA_trans[i, j, k] + UA_trans[i, j, k] -
  AT_trans[i, j, k] - AD_trans[i, j, k] - AU_trans[i, j, k] - A_smc_cleared[i, j, k] - A_death[i, j, k] -
  A_age[i, j, k] + A_age[i - 1, j, k]


update(U[1, 1:nh, 1:num_int]) <- U[i, j, k] + AU_trans[i, j, k] - UD_trans[i, j, k] - UT_trans[i, j, k] -
  US_trans[i, j, k] - U_smc_cleared[i, j, k] - UA_trans[i, j, k] - U_age[i, j, k] - U_death[i, j, k]
update(U[2:na, 1:nh, 1:num_int]) <- U[i, j, k] + AU_trans[i, j, k] - UD_trans[i, j, k] - UT_trans[i, j, k] -
  US_trans[i, j, k] - U_smc_cleared[i, j, k] - UA_trans[i, j, k] - U_age[i, j, k] - U_death[i, j, k] + U_age[i - 1, j, k]

update(P[1, 1:nh, 1:num_int]) <- P[i, j, k] + TP_trans[i, j, k] - PS_trans[i, j, k] - P_death[i, j, k] -
  P_age[i, j, k]
update(P[2:na, 1:nh, 1:num_int]) <- P[i, j, k] + TP_trans[i, j, k] - PS_trans[i, j, k] - P_death[i, j, k] -
  P_age[i, j, k] + P_age[i - 1, j, k]
...
FOI_lag[1:na, 1:nh, 1:num_int] <- EIR[i, j, k] * (if (IB[i, j, k] == 0)
  b0
  else
    b[i, j, k])
...
update(FOI_XL[, , , 1]) <- FOI_XL[i, j, k, 1] + dt * ((lag_rates / dE) *
                                                        FOI_lag[i, j, k] - (lag_rates / dE) * FOI_XL[i, j, k, 1])
update(FOI_XL[, , , 2:lag_rates]) <- FOI_XL[i, j, k, l] + dt * ((lag_rates /
                                                                   dE) * FOI_XL[i, j, k, l - 1] - (lag_rates / dE) * FOI_XL[i, j, k, l])
...
init_ICM_pre[1:nh, 1:num_int] <- PM * (ICA[age20l, i, j] + age_20_factor *
                                         (ICA[age20u, i, j] - ICA[age20l, i, j]))

update(ICM[1, 1:nh, 1:num_int]) <- ICM[i, j, k] + dt * (-1 / dCM * ICM[i, j, k] + (init_ICM_pre[j, k] -
                                                                                     ICM[i, j, k]) / x_I[i])
update(ICM[2:na, 1:nh, 1:num_int]) <- ICM[i, j, k] + dt * (-1 / dCM * ICM[i, j, k] - (ICM[i, j, k] -
                                                                                        ICM[i - 1, j, k]) / x_I[i])
...
update(ICA[1, 1:nh, 1:num_int]) <- ICA[i, j, k] + dt * (FOI_smc[i, j, k] /
                                                          (FOI_smc[i, j, k] * uCA + 1) - 1 / dCA * ICA[i, j, k] - ICA[i, j, k] / x_I[i])
update(ICA[2:na, 1:nh, 1:num_int]) <- ICA[i, j, k] + dt * (FOI_smc[i, j, k] /
                                                             (FOI_smc[i, j, k] * uCA + 1) - 1 / dCA * ICA[i, j, k] - (ICA[i, j, k] -
                                                                                                                        ICA[i - 1, j, k]) / x_I[i])
...
update(IB[1, 1:nh, 1:num_int]) <- IB[i, j, k] + dt * (EIR[i, j, k] / (EIR[i, j, k] * uB + 1) - IB[i, j, k] /
                                                        dB - IB[i, j, k] / x_I[i])
update(IB[2:na, 1:nh, 1:num_int]) <- IB[i, j, k] + dt * (EIR[i, j, k] /
                                                           (EIR[i, j, k] * uB + 1) - IB[i, j, k] / dB - (IB[i, j, k] - IB[i - 1, j, k]) /
                                                           x_I[i])
...
update(ID[1, 1:nh, 1:num_int]) <- ID[i, j, k] + dt * (FOI_smc[i, j, k] /
                                                        (FOI_smc[i, j, k] * uD + 1) - ID[i, j, k] / dID - ID[i, j, k] / x_I[i])
update(ID[2:na, 1:nh, 1:num_int]) <- ID[i, j, k] + dt * (FOI_smc[i, j, k] /
                                                           (FOI_smc[i, j, k] * uD + 1) - ID[i, j, k] / dID - (ID[i, j, k] - ID[i -
                                                                                                                                 1, j, k]) / x_I[i])
...
b[1:na, 1:nh, 1:num_int] <- b0 * ((1 - b1) / (1 + (IB[i, j, k] / IB0)^kB) +
                                    b1)
...
update(ince_delay[1]) <- ince_delay[1] + dt * (ince - (lag_ratesMos / delayMos_use) *
                                                 ince_delay[1])
update(ince_delay[2:lag_ratesMos]) <- ince_delay[i] + dt * ((lag_ratesMos /
                                                               delayMos_use) * ince_delay[i - 1] -
                                                              (lag_ratesMos /
                                                                 delayMos_use) * ince_delay[i]
)
...
lambda <- -0.5 * b_lambda + sqrt(0.25 * b_lambda^2 + gammaL * beta_larval *
                                   muLL * dEL / (2 * muEL * mum_use * dLL * (1 + dPL * muPL)))
K0 <- 2 * mv0 * dLL * mum_use * (1 + dPL * muPL) * gammaL * (lambda + 1) /
  (lambda / (muLL * dEL) - 1 / (muLL * dLL) - 1)
...
update(EL) <- if(EL + dt * (beta_larval * mv - muEL * (1 + (EL + LL) / KL) * EL - EL / dEL) < 0) 0
else (EL + dt * (beta_larval * mv - muEL * (1 + (EL + LL) / KL) * EL - EL / dEL))

# egg hatching - den. dep. mortality - maturing larvae
update(LL) <- if(LL + dt * (EL / dEL - muLL * (1 + gammaL * (EL + LL) / KL) * LL - LL / dLL) < 0) 0
else (LL + dt * (EL / dEL - muLL * (1 + gammaL * (EL + LL) / KL) * LL - LL / dLL))

# pupae - mortality - fully developed pupae
update(PL) <- if(PL + dt * (LL / dLL - muPL * PL - PL / dPL) < 0) 0
else (PL + dt * (LL / dLL - muPL * PL - PL / dPL))
...
n_prev[1:prev_dim] <- sum(S[min_age_prev[i]:max_age_prev[i], , ]) +
  sum(T[min_age_prev[i]:max_age_prev[i], , ]) + sum(D[min_age_prev[i]:max_age_prev[i], , ]) +
  sum(A[min_age_prev[i]:max_age_prev[i], , ]) + sum(U[min_age_prev[i]:max_age_prev[i], , ]) +
  sum(P[min_age_prev[i]:max_age_prev[i], , ])
